...
export default function DocumentsPage() {
    ...
    const handleDeleteDocument = async (id: number) => {
        ...
        try {
            await api.deleteDocument(id.toString());
            loadDocuments();
        } catch (error) {
    ...
    const stats = {
        ...
        views: documents.reduce((sum, doc) => sum + doc.view_count, 0),
        downloads: documents.reduce((sum, doc) => sum + doc.download_count, 0),
                    ...
                    <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                        {categories.map(cat => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg whitespace-nowrap transition ${selectedCategory === cat.value
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-4 text-xs text-gray-500">
                                                <span><i className="fas fa-eye mr-1"></i>{doc.view_count}</span>
                                                <span><i className="fas fa-download mr-1"></i>{doc.download_count}</span>
                                            </div>
                                        </div>
                                    </div>
